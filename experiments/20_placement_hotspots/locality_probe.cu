...
__global__ void locality_probe(unsigned long long base, unsigned int *out,
                               int pages, int entries_per_page, int stride) {
  ...
  unsigned long long address =
      base + (unsigned long long)page * (unsigned long long)stride * 4096ULL +
...
int main(int argc, char **argv) {
  ...
  int stride = argc > 4 ? atoi(argv[4]) : 1;
  if (stride < 1 || stride > 32) return 2;
  ...
  locality_probe<<<(threads + 255) / 256, 256>>>(base, out, pages, entries, stride);
